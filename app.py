...
"""
# ── No more sys.modules hack ─────────────────────────────────────────────────
# SECURITY_MASTER_URL now lives inside config.py so breeze_connect can find it
# without us injecting a fake module.
# ──────────────────────────────────────────────────────────────────────────────
...
def render_dashboard_tab():
    ...
    if option_chain["success"]:
        ...
        if not df.empty:
            ...
            st.dataframe(
                df[['strike_price', 'right', 'ltp', 'open_interest', 'volume',
...
def render_square_off_tab():
    ...
    display_cols = ['stock_code', 'expiry_date', 'strike_price', 'right',
                    'quantity', 'average_price', 'ltp']
    ...
    if option_positions:
        ...
        with col1:
            sq_order_type = st.radio(
                "Order Type", options=["Market", "Limit"],
        ...
        with col2:
            ...
            if sq_order_type == "Limit":
                sq_limit_price = st.number_input(
                    "Limit Price", min_value=0.0, step=0.05, key="sq_limit_price"
                )

        sq_qty = st.number_input(
            "Quantity to Square Off",
            min_value=1, max_value=qty, value=qty, key="sq_qty"
        )
...
def render_orders_tab():
    ...
    with col1:
        exchange_filter = st.selectbox(
            "Exchange", options=["All", "NFO", "BFO"], key="order_exchange"
        )
    with col2:
    ...
    with col3:
        to_date = st.date_input(
            "To Date", value=datetime.now().date(), key="order_to"
        )
    ...
    pending_orders = [
        ...
        if o.get("order_status", "").lower() in ["pending", "open"]
    ]
        ...
        with col2:
            if st.button("✏️ Modify Order", use_container_width=True):
                ...
                new_price = st.number_input(
                    ...
                    value=float(selected_order.get("price", 0)), step=0.05
                )
                new_qty = st.number_input(
                    "New Quantity", min_value=1,
...
def render_positions_tab():
    ...
    for pos in position_list:
        ...
        if qty != 0:
            pnl = (ltp - avg_price) * qty if qty > 0 else (avg_price - ltp) * abs(qty)
    ...
    with col1:
        st.metric("Total Positions",
                   len([p for p in position_list if int(p.get("quantity", 0)) != 0]))
    with col2:
        st.metric("Long Positions",
                   len([p for p in position_list if int(p.get("quantity", 0)) > 0]))
    with col3:
        st.metric("Short Positions",
                   len([p for p in position_list if int(p.get("quantity", 0)) < 0]))
    with col4:
    ...
    if active_positions:
        for pos in active_positions:
            with st.expander(
                f"{pos.get('stock_code')} {pos.get('strike_price')} "
